...
#.gp = as.genepop(.final) %>% {print(head(., 10)); .)
...
.conf = purrr::map_dfr(.infiles, wtl::read_boost_ini, .id='path') %>%
    select(path, toepad_select, morph_compe, migration_rate) %>%
    mutate(path=as.character(path))

ibd = .conf %>%
    pmap_dfr(function(path, ..., .parallel=TRUE) {
        message(path)
        pipeline(path)
    })
