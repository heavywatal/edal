---
title: "Simulation results 2014-11-03"
date: "2014-11-03"
output:
  html_document:
    toc: true
...
rmarkdown::render('~/edal/unnormalized.Rmd', output_dir=.outdir)
```
...
```{r plot, fig.width=9, fig.height=8}
.conf %>>%
    group_by(label) %>>%
    do({
        .gpl = llply(.$id, .run_grob, .parallel=TRUE)
        .grob = wtl::grid_grob(.gpl, 4, 1)
        print(.$id)
        print(.grob)
        .
    })
